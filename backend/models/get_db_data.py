...
def get_db_month_data(month):
        
    connection = psycopg2.connect(database="postgres",user="postgres",password="postgres",host="127.0.0.1",port='5432')
    cursor = connection.cursor()
    if month == "this":
        cursor.execute("SELECT song_name, artist, image_url, timestamp_column FROM  listened_list WHERE timestamp_column >= date_trunc('month', CURRENT_DATE);")
    else:
        cursor.execute("SELECT song_name, artist, image_url, timestamp_column FROM listened_list WHERE timestamp_column >= date_trunc('month', CURRENT_DATE - interval '1 month') AND timestamp_column < date_trunc('month', CURRENT_DATE);")
